  ...
  if m and m.group(1) in prefix_set:
    if(skip_global_checks):
      for nameless_value in nameless_values:
        ...
        if global_ir_value_re.search(input_line):
          return False
      return True
    return False
...
nameless_values = [
    ...
    NamelessValue(r'GLOB' , '@' , None           , r'@'            , r'[a-zA-Z0-9-_$"\\.]+' , None         , r'.+'                , True)  ,
...
def get_value_definition(var, match):
  # for backwards compatibility we check locals with '.*'
  if is_local_def_ir_value_match(match):
    return '[[' + get_value_name(var, get_nameless_check_prefix_from_ir_value_match(match)) + ':' + get_ir_prefix_from_ir_value_match(match)[0] + get_ir_prefix_from_ir_value_re_match(match) + ']]'
...
def add_checks(output_lines, comment_marker, prefix_list, func_dict, func_name, check_label_format, is_asm, is_analyze, global_vars_seen_dict):
  ...
  for p in prefix_list:
    ...
    for checkprefix in checkprefixes:
      ...
      global_vars_seen_before = global_vars_seen.keys()[:]
