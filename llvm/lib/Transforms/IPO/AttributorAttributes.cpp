...
struct AAIndirectCallInfoCallSite : public AAIndirectCallInfo {
  ...
  void initialize(Attributor &A) override {
    ...
    if (MD) {
      ...
      PotentialCallees.insert(
          A.getInfoCache().getIndirectlyCallableFunctions().begin(),
          A.getInfoCache().getIndirectlyCallableFunctions().end());
      errs() << "Added " << PotentialCallees.size() << "\n";
    }
