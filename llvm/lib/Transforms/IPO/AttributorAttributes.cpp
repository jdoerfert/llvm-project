...
struct AAInstanceInfoImpl : public AAInstanceInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto UsePred = [&](const Use &U, bool &Follow) {
      ...
      if (!UserI || isa<GetElementPtrInst>(UserI) || isa<BitCastInst>(UserI) ||
          isa<PHINode>(UserI) || isa<SelectInst>(UserI) ||
      ...
      if (isa<LoadInst>(UserI) || isa<CmpInst>(UserI) ||
          (isa<StoreInst>(UserI) && &UserI->getOperandUse(1) == &U))
        return true;
      if (auto *CB = dyn_cast<CallBase>(UserI)) {
        ...
        const auto &ArgInstanceInfoAA = A.getAAFor<AAInstanceInfo>(
            *this, IRPosition::callsite_argument(*CB, CB->getArgOperandNo(&U)),
            DepClassTy::OPTIONAL);
        if (ArgInstanceInfoAA.isAssumedUnique())
          return true;
        // if (Scope && !AA::isPotentiallyReachable(
        // A, *CB, *Scope, *this,
  ...
  const std::string getAsStr() const override {
    return isAssumedUnique() ? "<unique>" : "<unknown>";
  }
...
struct AAValueSimplifyImpl : AAValueSimplify {
  ...
  Value *manifestReplacementValue(Attributor &A, Instruction *CtxI) const {
    Value *NewV = SimplifiedAssociatedValue.getValueOr(
        PoisonValue::get(getAssociatedType()));
    if (NewV != &getAssociatedValue()) {
      ValueToValueMapTy VMap;
