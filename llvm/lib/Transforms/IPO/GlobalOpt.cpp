...
static bool CleanupConstantGlobalUsers(
  ...
  auto EnqueueUsers = [&WorkList](Value *Ptr, Constant *Init) {
    for (auto *U : Ptr->users())
      WorkList.push_back(WorkItem{U, Ptr, Init});
  };
  EnqueueUsers(FirstV, FirstInit);
