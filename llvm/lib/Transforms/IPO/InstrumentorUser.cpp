...
struct ExtendedAllocaIO : public AllocaIO {
  ...
  void init(InstrumentationConfig &IConf, InstrumentorIRBuilderTy &IIRB) {
    ...
    IRTArgs.push_back(IRTArg(IIRB.PtrTy, "function_name", "The function name.",
...
void InstrumentorUserConfig::populate(InstrumentorIRBuilderTy &IIRB) {
  ...
  CICConfig.ArgFilter = [&](Use &Op) { return Op->getType()->isPointerTy(); };
...
PreservedAnalyses InstrumentorUserPass::run(Module &M,
