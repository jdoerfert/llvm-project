...
struct AAExecutionDomainFunction : public AAExecutionDomain {
  ...
  ~AAExecutionDomainFunction() {
    delete RPOT;
    for (auto &It : BEDMap) {
      if (It.second.EncounteredAssumes)
        It.second.EncounteredAssumes->~SmallPtrSet();
      if (It.second.AlignedBarriers)
        It.second.AlignedBarriers->~SmallPtrSet();
    }
    for (auto &It : CEDMap) {
      if (It.second.EncounteredAssumes)
        It.second.EncounteredAssumes->~SmallPtrSet();
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto HandleAlignedBarrier = [&](CallBase *CB) {
      ...
      if (ED.EncounteredAssumes && (CB || ED.AlignedBarriers))
...
void AAExecutionDomainFunction::mergeInPredecessorBarriersAndAssumptions(
  ...
  if (PredED.EncounteredAssumes && !PredED.EncounteredAssumes->empty()) {
    for (auto *EA : *PredED.EncounteredAssumes)
      ED.addAssumeInst(A, *EA);
  }

  if (PredED.AlignedBarriers && !PredED.AlignedBarriers->empty()) {
