...
static bool isPotentiallyReachable(
    Attributor &A, const Instruction &FromI, const Instruction *ToI,
    const Function &ToFn, const AbstractAttribute &QueryingAA,
    const AA::BlockExclusionSetTy *ExclusionSet, const BasicBlock *AllowBlock,
  ...
  while (!Worklist.empty()) {
    ...
    const auto &ToReachabilityAA = A.getAAFor<AAReachability>(
        QueryingAA, IRPosition::function(ToFn), DepClassTy::OPTIONAL);
    const Instruction &EntryI = ToFn.getEntryBlock().front();
    bool Result = ToReachabilityAA.isAssumedReachable(A, EntryI, *ToI,
                                                      ExclusionSet, AllowBlock);
    ...
    auto ReturnInstCB = [&](Instruction &Ret) {
      bool Result = ReachabilityAA.isAssumedReachable(A, *CurFromI, Ret,
                                                      ExclusionSet, AllowBlock);
...
bool AA::isPotentiallyReachable(
  ...
  return ::isPotentiallyReachable(A, FromI, &ToI, *ToFn, QueryingAA,
                                  ExclusionSet, AllowBlock, GoBackwardsCB);
}
