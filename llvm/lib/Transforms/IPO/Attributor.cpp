  ...
  void createReplacementValues(Type *PrivType, AbstractCallSite ACS,
    ...
    if (auto *PrivStructType = dyn_cast<StructType>(PrivType)) {
      ...
      for (unsigned u = 0, e = PrivStructType->getNumElements(); u < e; u++) {
        ...
        LoadInst *LI = new LoadInst(PointeeTy, Ptr, "", IP);
        LI->setAlignment(Ptr->getPointerAlignment(DL));
        ReplacementValues.push_back(LI);
      }
    } else if (auto *PrivArrayType = dyn_cast<ArrayType>(PrivType)) {
      ...
      for (unsigned u = 0, e = PrivArrayType->getNumElements(); u < e; u++) {
        Value *Ptr =
            constructPointer(PointeePtrTy, Base, u * PointeeTySize, IRB, DL);
        LoadInst *LI = new LoadInst(PointeePtrTy, Ptr, "", IP);
        LI->setAlignment(Ptr->getPointerAlignment(DL));
        ReplacementValues.push_back(LI);
      }
    } else {
      LoadInst *LI = new LoadInst(PrivType, Base, "", IP);
      LI->setAlignment(Base->getPointerAlignment(DL));
      ReplacementValues.push_back(LI);
    }
  }
