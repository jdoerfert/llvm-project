  ...
  while (!Worklist.empty()) {
    ...
    if (auto *SI = dyn_cast<StoreInst>(U->getUser())) {
      if (&SI->getOperandUse(0) == U) {
        ...
        auto *LoadsFromStoreAA = getAAFor<AALoadsFromStore>(
            QueryingAA, IRPosition::inst(*SI), DepClassTy::OPTIONAL);
        if (LoadsFromStoreAA &&
            LoadsFromStoreAA->getPotentialCopiesOfStoredValue(
                *this, PotentialCopies, UsedAssumedInformation,
                /* OnlyExact */ true)) {
          DEBUG_WITH_TYPE(VERBOSE_DEBUG_TYPE,
                          dbgs()
