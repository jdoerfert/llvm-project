...
static bool runLightweightAttributorOnFunctions(
  ...
  DenseSet<const char *> Allowed(
      {&AAWillReturn::ID, &AANoUnwind::ID, &AANoRecurse::ID, &AANoSync::ID,
       ...
       &AANonNull::ID, &AAUnderlyingObjects::ID, &AANoCapture::ID,
       &AANoFPClass::ID, &AANoAlias::ID, &AAPotentialValues::ID,
       &AANoReturn::ID});
  AC.Allowed = &Allowed;
...

  {
    ...
    for (Function *F : Functions) {
      ...
      // We look at internal functions only on-demand but if any use is not a
      // direct call or outside the current set of analyzed functions, we have
      // to do it eagerly. If we do not perform liveness analysis, everything is
      // live.
