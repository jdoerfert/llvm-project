...
struct IntegerStateBase : public AbstractState {
  ...
  virtual base_t getKnown() const { return Known; }

  /// Return the assumed state encoding.
  virtual base_t getAssumed() const { return Assumed; }
...
struct BitIntegerState : public IntegerStateBase<uint32_t, ~0u, 0> {
  ...
  virtual bool isKnown(base_t BitsEncoding) const {
    return (Known & BitsEncoding) == BitsEncoding;
  }

  /// Return true if the bits set in \p BitsEncoding are "assumed bits".
  virtual bool isAssumed(base_t BitsEncoding) const {
