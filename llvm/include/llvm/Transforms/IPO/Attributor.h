...
struct AAExecutionDomain
  ...
  struct ExecutionDomainTy {
    ...
    void addAssumeInst(Attributor &A, AssumeInst &AI) {
      if (!IsReachedFromAlignedBarrierOnly || EncounteredNonLocalSideEffect)
        return;
      if (!EncounteredAssumes)
        EncounteredAssumes = new (A.Allocator) AssumesSetTy();
    ...
    void addAlignedBarrier(Attributor &A, CallBase &CB) {
      if (!IsReachedFromAlignedBarrierOnly || EncounteredNonLocalSideEffect)
        return;
      if (!AlignedBarriers)
        AlignedBarriers = new (A.Allocator) BarriersSetTy();
