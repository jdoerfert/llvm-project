...
template <typename base_ty> struct SetState : public AbstractState {
  ...
  struct SetContents {
    ...
    SetContents(const DenseSet<base_ty> &Assumptions)
        : Universal(false), Set(Assumptions) {}

    SetContents(bool Universal, const DenseSet<base_ty> &Assumptions)
        : Universal(Universal), Set(Assumptions) {}

    const DenseSet<base_ty> &getSet() const { return Set; }
  ...
  SetState(const DenseSet<base_ty> &Known)
  ...
  bool getIntersection(const SetContents &RHS) {
    ...
    return Changed != Assumed.getSet().size();
  }
