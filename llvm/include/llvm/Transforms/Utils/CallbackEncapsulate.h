...
/// argument promotion as performed by the Attributor, to modify the passed
/// arguments without changing the interface of \p Called and \p Callee. This
/// can be good for two reasons:
///
...
/// tied together through metadata (see !rpl_cs and !rpl_acs) as shown in the
/// example below. In addition, the original arguments are tied to the new
/// callback call via metadata (see !rpl_cs) to indicate that their use can
/// be ignored in favor of handling the callback call.
///
...
/// If the calls are modified and the metadata is dropped the encapuslation is
/// is lost.
///
///  ------------------------------- Before ------------------------------------
///    void foo() {
...
///
///    void foo() {
///      // metadata !rpl_cs !{!1, i32 2, i32 0, i32 1}
...
bool isDirectCallSiteUseReplacedByAbstractCallSiteUse(ImmutableCallSite CS,
                                                      const Use &U);
