...
; RUN: opt -disable-output -print-mustexecute -print-mustexecute-use-explorer %s 2>&1 | FileCheck %s
...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    br label [[LOOP:%.*]] ; no mustexec loop
; CHECK:       loop: ; preds = [[BACKEDGE:%.*]], [[ENTRY:%.*]]
...
; CHECK:       exit: ; preds = [[BACKEDGE]]
; CHECK-NEXT:    ret i1 false ; no mustexec loop
; CHECK:       never1: ; preds = [[LOOP]]
; CHECK-NEXT:    unreachable ; no mustexec loop
; CHECK:       never2: ; preds = [[NEXT]]
; CHECK-NEXT:    unreachable ; no mustexec loop
;
entry:
  br label %loop
