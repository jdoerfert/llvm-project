...
; RUN: opt < %s -passes=function-attrs -S | FileCheck --check-prefixes=COMMON,CHECK %s
; RUN: opt < %s -passes=light-attributor -S | FileCheck --check-prefixes=COMMON,ATTRIBUTOR %s
...
; ATTRIBUTOR-SAME: (ptr [[X1_2:%.*]], ptr nocapture readonly [[Y1_2:%.*]], ptr [[Z1_2:%.*]]) {
; ATTRIBUTOR-NEXT:    call void (ptr, ptr, ...) @test1_1(ptr [[X1_2]], ptr nocapture readonly [[Y1_2]], ptr [[Z1_2]])
...
; TODO: Missing with attributor: mustprogress, argmem: none, inaccessiblemem: none
...
;
; ATTRIBUTOR: Function Attrs: nofree norecurse nosync nounwind willreturn memory(write)
; ATTRIBUTOR-LABEL: define {{[^@]+}}@test2
; ATTRIBUTOR-SAME: (ptr nofree readnone returned [[P:%.*]]) #[[ATTR0:[0-9]+]] {
; ATTRIBUTOR-NEXT:    store i32 0, ptr @x, align 4
...
; ATTRIBUTOR-SAME: (ptr nocapture readonly [[P:%.*]]) #[[ATTR2:[0-9]+]] {
; ATTRIBUTOR-NEXT:    call void @test4_1(ptr nocapture readonly [[P]])
; ATTRIBUTOR-NEXT:    ret void
;
...
; CHECK-NEXT:    ret void
;
; ATTRIBUTOR: Function Attrs: nofree norecurse nosync nounwind willreturn memory(argmem: write)
; ATTRIBUTOR-LABEL: define {{[^@]+}}@test5
; ATTRIBUTOR-SAME: (ptr nocapture nofree writeonly [[P:%.*]], ptr nofree writeonly [[Q:%.*]]) #[[ATTR3:[0-9]+]] {
...
; ATTRIBUTOR-SAME: (ptr nocapture nofree writeonly [[P:%.*]], ptr nofree [[Q:%.*]]) {
; ATTRIBUTOR-NEXT:    store ptr [[Q]], ptr [[P]], align 8
...
; ATTRIBUTOR-SAME: (ptr nocapture nofree writeonly inalloca(i32) [[A:%.*]]) #[[ATTR1]] {
; ATTRIBUTOR-NEXT:    ret void
;
  ret void
}
...
; ATTRIBUTOR-SAME: (ptr nocapture nofree writeonly preallocated(i32) [[A:%.*]]) #[[ATTR1]] {
; ATTRIBUTOR-NEXT:    ret void
;
  ret void
}
...
; ATTRIBUTOR-SAME: (ptr nofree readnone returned [[P:%.*]]) #[[ATTR1]] {
...
; ATTRIBUTOR-NEXT:  entry:
; ATTRIBUTOR-NEXT:    [[CALL:%.*]] = call ptr @test8_1(ptr nofree readnone [[P]]) #[[ATTR11:[0-9]+]]
...
; ATTRIBUTOR-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[VAL]], <4 x ptr> [[PTRS]], i32 4, <4 x i1> <i1 true, i1 false, i1 true, i1 false>) #[[ATTR12:[0-9]+]]
; ATTRIBUTOR-NEXT:    ret void
;
...
;
; ATTRIBUTOR: Function Attrs: nofree norecurse nosync nounwind willreturn memory(read)
; ATTRIBUTOR-LABEL: define {{[^@]+}}@test10
; ATTRIBUTOR-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR6:[0-9]+]] {
; ATTRIBUTOR-NEXT:    [[RES:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> [[PTRS]], i32 4, <4 x i1> <i1 true, i1 false, i1 true, i1 false>, <4 x i32> undef) #[[ATTR12]]
...
; CHECK-NEXT:    ret <4 x i32> [[RES]]
;
; ATTRIBUTOR: Function Attrs: nounwind memory(argmem: read)
; ATTRIBUTOR-LABEL: define {{[^@]+}}@test11_2
; ATTRIBUTOR-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR7:[0-9]+]] {
; ATTRIBUTOR-NEXT:    [[RES:%.*]] = call <4 x i32> @test11_1(<4 x ptr> [[PTRS]])
; ATTRIBUTOR-NEXT:    ret <4 x i32> [[RES]]
;
...
;
; ATTRIBUTOR: Function Attrs: nofree norecurse nounwind willreturn memory(argmem: readwrite)
...
; ATTRIBUTOR-SAME: (ptr nocapture nofree readnone [[IGNORED:%.*]], ptr [[ESCAPED_THEN_WRITTEN:%.*]]) {
; ATTRIBUTOR-NEXT:    [[ADDR:%.*]] = alloca ptr, align 8
; ATTRIBUTOR-NEXT:    call void @escape_readnone_ptr(ptr [[ADDR]], ptr [[ESCAPED_THEN_WRITTEN]])
...
; ATTRIBUTOR-SAME: (ptr nocapture nofree readnone [[IGNORED:%.*]], ptr [[ESCAPED_THEN_WRITTEN:%.*]]) {
; ATTRIBUTOR-NEXT:    [[ADDR:%.*]] = alloca ptr, align 8
; ATTRIBUTOR-NEXT:    call void @escape_readonly_ptr(ptr [[ADDR]], ptr [[ESCAPED_THEN_WRITTEN]])
; ATTRIBUTOR-NEXT:    [[ADDR_LD:%.*]] = load ptr, ptr [[ADDR]], align 8
; ATTRIBUTOR-NEXT:    store i8 0, ptr [[ADDR_LD]], align 1
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr nocapture [[P:%.*]], ptr nocapture nofree [[F:%.*]]) {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr nocapture readnone [[P]])
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr [[P:%.*]], ptr nocapture nofree [[F:%.*]]) {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr readnone [[P]])
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr readonly [[P:%.*]], ptr nocapture nofree readonly [[F:%.*]]) #[[ATTR2]] {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr readnone [[P]]) #[[ATTR2]]
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr nocapture [[P:%.*]], ptr nocapture nofree [[F:%.*]]) {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr nocapture readonly [[P]])
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr [[P:%.*]], ptr nocapture nofree [[F:%.*]]) {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr readonly [[P]])
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr readonly [[P:%.*]], ptr nocapture nofree readonly [[F:%.*]]) #[[ATTR2]] {
; ATTRIBUTOR-NEXT:    call void [[F]](ptr readonly [[P]]) #[[ATTR2]]
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-NEXT:    call void @readnone_param(ptr nocapture [[P]]) [ "deopt"() ]
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr nocapture [[P:%.*]]) {
; ATTRIBUTOR-NEXT:    call void @readnone_param(ptr nocapture [[P]]) [ "unknown"() ]
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr nocapture [[P:%.*]]) {
; ATTRIBUTOR-NEXT:    call void @readonly_param(ptr nocapture [[P]]) [ "deopt"() ]
; ATTRIBUTOR-NEXT:    ret void
;
...
; ATTRIBUTOR-SAME: (ptr nocapture [[P:%.*]]) {
; ATTRIBUTOR-NEXT:    call void @readonly_param(ptr nocapture [[P]]) [ "unknown"() ]
; ATTRIBUTOR-NEXT:    ret void
;
