...
; RUN: opt %s -globalopt -argpromotion -sroa -S | FileCheck %s --check-prefix=ARGPROMOTION
; RUN: opt %s -passes='module(globalopt),cgscc(argpromotion),function(sroa)' -S | FileCheck %s --check-prefix=ARGPROMOTION
...
; ARGPROMOTION-SAME: (%struct.ss* [[A:%.*]], %struct.ss* [[B:%.*]]) unnamed_addr #0
; ARGPROMOTION-NEXT:  entry:
