...
; RUN: opt -S -passes='attributor' -aa-pipeline='basic-aa' -attributor-disable=false -attributor-max-iterations-verify -attributor-max-iterations=8 < %s | FileCheck %s --check-prefixes=ALL,ATTRIBUTOR
...
; ATTRIBUTOR-NEXT:    [[C:%.*]] = call i32 @test()
; ATTRIBUTOR-NEXT:    ret i32 [[C]]
>>>>>>> 2bf5733254a... FIXES FIXES FIXES
;
  %A = alloca i32
...
;
; ATTRIBUTOR-LABEL: define {{[^@]+}}@callercaller()
; ATTRIBUTOR-NEXT:    [[X:%.*]] = call i32 @caller()
