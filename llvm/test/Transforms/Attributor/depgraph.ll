...
; RUN: opt -passes=attributor-cgscc -disable-output -attributor-use-must-be-executed-context -attributor-print-dep < %s 2>&1 | FileCheck %s --check-prefixes=GRAPH
; RUN: opt -passes=attributor-cgscc -disable-output -attributor-use-must-be-executed-context -attributor-dump-dep-graph -attributor-depgraph-dot-filename-prefix=%t < %s 2>/dev/null
...
; CHECK-SAME: (i32* nofree readonly align 16 [[TMP0:%.*]]) [[ATTR0:#.*]] {
...
; CHECK-NEXT:    [[TMP6:%.*]] = call i32* @checkAndAdvance(i32* nofree nonnull readonly align 16 [[TMP5]]) [[ATTR1:#.*]]
