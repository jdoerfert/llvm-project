...
; TUNIT-SAME: (i32 [[I:%.*]], i1 [[F:%.*]], i32 [[N:%.*]]) {
; TUNIT-NEXT:  entry:
; TUNIT-NEXT:    [[C:%.*]] = icmp ne i32 [[I]], -2134
; TUNIT-NEXT:    br i1 [[C]], label [[DO:%.*]], label [[EXIT:%.*]]
; TUNIT:       exit:
; TUNIT-NEXT:    [[C1:%.*]] = icmp ne i32 [[I]], -42
; TUNIT-NEXT:    br i1 [[C1]], label [[EXIT2:%.*]], label [[EXIT]]
; TUNIT:       cont:
; TUNIT-NEXT:    call void @dummy(i1 [[F]]) #[[ATTR2:[0-9]+]]
; TUNIT-NEXT:    br label [[EXIT2]]
; TUNIT:       do:
; TUNIT-NEXT:    call void @dummy(i1 [[F]]) #[[ATTR2]]
; TUNIT-NEXT:    [[CONSUME:%.*]] = call i32 @exit()
; TUNIT-NEXT:    call void @llvm.assume(i1 noundef [[F]]) #[[ATTR4:[0-9]+]]
; TUNIT-NEXT:    [[COND:%.*]] = icmp eq i1 [[F]], false
; TUNIT-NEXT:    br i1 [[COND]], label [[EXIT]], label [[CONT:%.*]]
; TUNIT:       exit2:
; TUNIT-NEXT:    ret i32 30
;
; CGSCC-LABEL: define {{[^@]+}}@test4
; CGSCC-SAME: (i32 [[I:%.*]], i1 [[F:%.*]], i32 [[N:%.*]]) {
; CGSCC-NEXT:  entry:
; CGSCC-NEXT:    [[C:%.*]] = icmp ne i32 [[I]], -2134
; CGSCC-NEXT:    br i1 [[C]], label [[DO:%.*]], label [[EXIT:%.*]]
; CGSCC:       exit:
; CGSCC-NEXT:    [[C1:%.*]] = icmp ne i32 [[I]], -42
; CGSCC-NEXT:    br i1 [[C1]], label [[EXIT2:%.*]], label [[EXIT]]
; CGSCC:       cont:
; CGSCC-NEXT:    call void @dummy(i1 [[F]]) #[[ATTR2:[0-9]+]]
; CGSCC-NEXT:    br label [[EXIT2]]
; CGSCC:       do:
; CGSCC-NEXT:    call void @dummy(i1 [[F]]) #[[ATTR2]]
; CGSCC-NEXT:    [[CONSUME:%.*]] = call i32 @exit()
; CGSCC-NEXT:    call void @llvm.assume(i1 noundef [[F]])
; CGSCC-NEXT:    [[COND:%.*]] = icmp eq i1 [[F]], false
; CGSCC-NEXT:    br i1 [[COND]], label [[EXIT]], label [[CONT:%.*]]
; CGSCC:       exit2:
; CGSCC-NEXT:    ret i32 30
;
entry:
  %c = icmp ne i32 %i, -2134
  br i1 %c, label %do, label %exit
...
; TUNIT: attributes #[[ATTR1:[0-9]+]] = { nocallback nofree nosync nounwind willreturn memory(inaccessiblemem: readwrite) }
; TUNIT: attributes #[[ATTR2]] = { nounwind }
; TUNIT: attributes #[[ATTR3:[0-9]+]] = { nocallback nofree nosync willreturn }
; TUNIT: attributes #[[ATTR4]] = { memory(inaccessiblemem: readwrite) }
;.
; CGSCC: attributes #[[ATTR0]] = { mustprogress nofree norecurse nosync nounwind willreturn memory(none) }
; CGSCC: attributes #[[ATTR1:[0-9]+]] = { nocallback nofree nosync nounwind willreturn memory(inaccessiblemem: readwrite) }
; CGSCC: attributes #[[ATTR2]] = { nounwind }
; CGSCC: attributes #[[ATTR3:[0-9]+]] = { nocallback nofree nosync willreturn }
;.
