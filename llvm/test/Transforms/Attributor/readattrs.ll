...
; CHECK-SAME: (ptr [[X1_2:%.*]], ptr nofree readonly [[Y1_2:%.*]], ptr [[Z1_2:%.*]]) {
...
; CHECK-SAME: (ptr nocapture nofree noundef nonnull writeonly align 8 dereferenceable(8) [[P:%.*]], ptr nofree readnone [[Q:%.*]]) #[[ATTR3:[0-9]+]] {
...
; CHECK-SAME: (ptr nocapture nofree nonnull writeonly inalloca(i32) dereferenceable(4) [[A:%.*]]) #[[ATTR3]] {
; CHECK-NEXT:    ret void
;
  ret void
}
...
; CGSCC-NEXT:  entry:
; CGSCC-NEXT:    [[CALL:%.*]] = call align 4 ptr @test8_1(ptr noalias nofree readnone [[P]]) #[[ATTR16:[0-9]+]]
...
; TUNIT-SAME: (<4 x ptr> [[PTRS:%.*]], <4 x i32> [[VAL:%.*]]) #[[ATTR5:[0-9]+]] {
; TUNIT-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[VAL]], <4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>) #[[ATTR15:[0-9]+]]
; TUNIT-NEXT:    ret void
;
; CGSCC: Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(write)
; CGSCC-LABEL: define {{[^@]+}}@test9
; CGSCC-SAME: (<4 x ptr> [[PTRS:%.*]], <4 x i32> [[VAL:%.*]]) #[[ATTR6:[0-9]+]] {
; CGSCC-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[VAL]], <4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>) #[[ATTR17:[0-9]+]]
; CGSCC-NEXT:    ret void
;
...
; TUNIT-NEXT:    [[RES:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>, <4 x i32> undef) #[[ATTR16:[0-9]+]]
...
; CGSCC-NEXT:    [[RES:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> [[PTRS]], i32 noundef 4, <4 x i1> noundef <i1 true, i1 false, i1 true, i1 false>, <4 x i32> undef) #[[ATTR17]]
...
; TUNIT-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR9]] {
; TUNIT-NEXT:    [[RES:%.*]] = call <4 x i32> @test12_1(<4 x ptr> [[PTRS]]) #[[ATTR9]]
; TUNIT-NEXT:    ret <4 x i32> [[RES]]
;
; CGSCC: Function Attrs: nounwind memory(none)
; CGSCC-LABEL: define {{[^@]+}}@test12_2
; CGSCC-SAME: (<4 x ptr> [[PTRS:%.*]]) #[[ATTR10]] {
; CGSCC-NEXT:    [[RES:%.*]] = call <4 x i32> @test12_1(<4 x ptr> [[PTRS]]) #[[ATTR10]]
; CGSCC-NEXT:    ret <4 x i32> [[RES]]
;
...
; TUNIT: Function Attrs: memory(readwrite, argmem: write)
; TUNIT-LABEL: define {{[^@]+}}@unsound_readnone
; TUNIT-SAME: (ptr nocapture nofree readnone [[IGNORED:%.*]], ptr nofree readnone [[ESCAPED_THEN_WRITTEN:%.*]]) #[[ATTR12:[0-9]+]] {
; TUNIT-NEXT:    [[ADDR:%.*]] = alloca ptr, align 8
; TUNIT-NEXT:    call void @escape_readnone_ptr(ptr noundef nonnull align 8 dereferenceable(8) [[ADDR]], ptr noalias nofree readnone [[ESCAPED_THEN_WRITTEN]])
; TUNIT-NEXT:    [[ADDR_LD:%.*]] = load ptr, ptr [[ADDR]], align 8
; TUNIT-NEXT:    store i8 0, ptr [[ADDR_LD]], align 1
; TUNIT-NEXT:    ret void
;
; CGSCC: Function Attrs: memory(readwrite, argmem: write)
; CGSCC-LABEL: define {{[^@]+}}@unsound_readnone
; CGSCC-SAME: (ptr nocapture nofree readnone [[IGNORED:%.*]], ptr nofree readnone [[ESCAPED_THEN_WRITTEN:%.*]]) #[[ATTR13:[0-9]+]] {
; CGSCC-NEXT:    [[ADDR:%.*]] = alloca ptr, align 8
; CGSCC-NEXT:    call void @escape_readnone_ptr(ptr noundef nonnull align 8 dereferenceable(8) [[ADDR]], ptr noalias nofree readnone [[ESCAPED_THEN_WRITTEN]])
; CGSCC-NEXT:    [[ADDR_LD:%.*]] = load ptr, ptr [[ADDR]], align 8
; CGSCC-NEXT:    store i8 0, ptr [[ADDR_LD]], align 1
; CGSCC-NEXT:    ret void
;
  %addr = alloca ptr
...
; CHECK-SAME: (ptr nocapture nofree readnone [[IGNORED:%.*]], ptr nofree readonly [[ESCAPED_THEN_WRITTEN:%.*]]) {
...
; CHECK-SAME: (ptr noalias nonnull readonly byval(i8) dereferenceable(1) [[WRITTEN:%.*]]) #[[ATTR2]] {
...
; CHECK-SAME: (ptr noalias nocapture nofree noundef nonnull readnone byval(i8) dereferenceable(1) [[WRITTEN:%.*]]) #[[ATTR1]] {
...
; TUNIT-SAME: (ptr noalias nonnull readnone byval(i8) dereferenceable(1) [[WRITTEN:%.*]]) #[[ATTR13:[0-9]+]] {
...
; CGSCC-SAME: (ptr noalias nonnull readnone byval(i8) dereferenceable(1) [[WRITTEN:%.*]]) #[[ATTR14:[0-9]+]] {
...
; TUNIT-SAME: (ptr nocapture nonnull readonly [[READ_ONLY:%.*]]) #[[ATTR2]] {
; TUNIT-NEXT:    call void @byval_not_readonly_1(ptr noalias nocapture nonnull readonly byval(i8) [[READ_ONLY]]) #[[ATTR2]]
; TUNIT-NEXT:    call void @byval_not_readnone_1(ptr noalias nocapture nonnull byval(i8) [[READ_ONLY]])
; TUNIT-NEXT:    call void @byval_no_fnarg(ptr noalias nocapture nofree noundef nonnull readnone byval(i8) [[READ_ONLY]]) #[[ATTR17:[0-9]+]]
; TUNIT-NEXT:    ret void
;
; CGSCC: Function Attrs: memory(read)
; CGSCC-LABEL: define {{[^@]+}}@testbyval
; CGSCC-SAME: (ptr nocapture nofree noundef nonnull readonly dereferenceable(1) [[READ_ONLY:%.*]]) #[[ATTR2]] {
; CGSCC-NEXT:    call void @byval_not_readonly_1(ptr noalias nocapture nofree noundef nonnull readonly byval(i8) dereferenceable(1) [[READ_ONLY]]) #[[ATTR2]]
; CGSCC-NEXT:    call void @byval_not_readnone_1(ptr noalias nocapture nofree noundef nonnull byval(i8) dereferenceable(1) [[READ_ONLY]])
; CGSCC-NEXT:    call void @byval_no_fnarg(ptr noalias nocapture nofree noundef nonnull readnone byval(i8) dereferenceable(1) [[READ_ONLY]]) #[[ATTR18:[0-9]+]]
; CGSCC-NEXT:    ret void
;
...
; TUNIT-SAME: (ptr nocapture nofree readonly [[PTR:%.*]]) #[[ATTR14:[0-9]+]] {
; TUNIT-NEXT:    [[CALL_PTR:%.*]] = call ptr @maybe_returned_ptr(ptr nofree readonly [[PTR]]) #[[ATTR14]]
; TUNIT-NEXT:    [[CALL_VAL:%.*]] = call i8 @maybe_returned_val(ptr readonly [[CALL_PTR]]) #[[ATTR14]]
; TUNIT-NEXT:    ret void
;
; CGSCC: Function Attrs: nounwind memory(read)
; CGSCC-LABEL: define {{[^@]+}}@ptr_uses
; CGSCC-SAME: (ptr nocapture nofree readonly [[PTR:%.*]]) #[[ATTR15:[0-9]+]] {
; CGSCC-NEXT:    [[CALL_PTR:%.*]] = call ptr @maybe_returned_ptr(ptr nofree readonly [[PTR]]) #[[ATTR15]]
; CGSCC-NEXT:    [[CALL_VAL:%.*]] = call i8 @maybe_returned_val(ptr readonly [[CALL_PTR]]) #[[ATTR15]]
; CGSCC-NEXT:    ret void
;
...
;.
; TUNIT: attributes #[[ATTR0]] = { mustprogress nofree norecurse nosync nounwind willreturn memory(write, argmem: none, inaccessiblemem: none) }
...
; TUNIT: attributes #[[ATTR10:[0-9]+]] = { nounwind memory(argmem: readwrite) }
; TUNIT: attributes #[[ATTR11]] = { mustprogress nofree norecurse nounwind willreturn memory(argmem: readwrite) }
; TUNIT: attributes #[[ATTR12]] = { memory(readwrite, argmem: write) }
; TUNIT: attributes #[[ATTR13]] = { memory(none) }
; TUNIT: attributes #[[ATTR14]] = { nounwind memory(read) }
; TUNIT: attributes #[[ATTR15]] = { nofree willreturn memory(write) }
; TUNIT: attributes #[[ATTR16]] = { nofree willreturn memory(read) }
; TUNIT: attributes #[[ATTR17]] = { nounwind memory(argmem: write) }
;.
; CGSCC: attributes #[[ATTR0]] = { mustprogress nofree norecurse nosync nounwind willreturn memory(write, argmem: none, inaccessiblemem: none) }
...
; CGSCC: attributes #[[ATTR10]] = { nounwind memory(none) }
; CGSCC: attributes #[[ATTR11:[0-9]+]] = { nounwind memory(argmem: readwrite) }
; CGSCC: attributes #[[ATTR12]] = { mustprogress nofree norecurse nounwind willreturn memory(argmem: readwrite) }
; CGSCC: attributes #[[ATTR13]] = { memory(readwrite, argmem: write) }
; CGSCC: attributes #[[ATTR14]] = { memory(none) }
; CGSCC: attributes #[[ATTR15]] = { nounwind memory(read) }
; CGSCC: attributes #[[ATTR16]] = { nofree willreturn memory(none) }
; CGSCC: attributes #[[ATTR17]] = { nofree willreturn }
; CGSCC: attributes #[[ATTR18]] = { nounwind memory(argmem: write) }
;.
