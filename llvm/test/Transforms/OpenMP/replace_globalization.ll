...
; CHECK-NEXT:    call void @use.internalized(ptr nofree readnone [[X]]) #[[ATTR7:[0-9]+]]
...
; CHECK:       master1:
; CHECK-NEXT:    call void @use.internalized(ptr nofree readnone addrspacecast (ptr addrspace(3) @x_shared to ptr)) #[[ATTR7]]
...
; CHECK:       master2:
; CHECK-NEXT:    call void @use.internalized(ptr nofree readnone addrspacecast (ptr addrspace(3) @y_shared to ptr)) #[[ATTR7]]
...
; CHECK-NEXT:    call void @use.internalized(ptr nofree readnone [[Z]]) #[[ATTR7]]
...
;
;
; CHECK: Function Attrs: nofree norecurse nosync nounwind memory(write, argmem: none, inaccessiblemem: none)
; CHECK-LABEL: define {{[^@]+}}@use.internalized
; CHECK-SAME: (ptr nofree readnone [[X:%.*]]) #[[ATTR1:[0-9]+]] {
; CHECK-NEXT:  entry:
...
;
;
; CHECK: Function Attrs: norecurse nosync nounwind allocsize(0) memory(read, inaccessiblemem: none)
...
; CHECK: attributes #[[ATTR1]] = { nofree norecurse nosync nounwind memory(write, argmem: none, inaccessiblemem: none) }
; CHECK: attributes #[[ATTR2]] = { norecurse nosync nounwind allocsize(0) memory(read, inaccessiblemem: none) }
...
; CHECK: attributes #[[ATTR6]] = { nounwind memory(read, inaccessiblemem: none) }
; CHECK: attributes #[[ATTR7]] = { nosync nounwind memory(write, argmem: none, inaccessiblemem: none) }
