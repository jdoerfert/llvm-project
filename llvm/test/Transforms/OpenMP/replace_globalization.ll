...
define void @baz_spmd() {
  %c = call i32 @__kmpc_target_init(%struct.ident_t* @1, i1 true, i1 true, i1 true)
  call void @unknown_no_openmp()
  %c0 = icmp eq i32 %c, -1
...
; CHECK:       master1:
; CHECK-NEXT:    call void @use.internalized(i8* nofree addrspacecast (i8 addrspace(3)* getelementptr inbounds ([16 x i8], [16 x i8] addrspace(3)* @x, i32 0, i32 0) to i8*)) #[[ATTR4]]
; CHECK-NEXT:    br label [[NEXT:%.*]]
; CHECK:       next:
; CHECK-NEXT:    call void @unknown_no_openmp()
; CHECK-NEXT:    br label [[MASTER2:%.*]]
; CHECK:       master2:
; CHECK-NEXT:    call void @use.internalized(i8* nofree addrspacecast (i8 addrspace(3)* getelementptr inbounds ([4 x i8], [4 x i8] addrspace(3)* @y, i32 0, i32 0) to i8*)) #[[ATTR4]]
...
; CHECK-NEXT:    [[C:%.*]] = call i32 @__kmpc_target_init(%struct.ident_t* @[[GLOB1]], i1 true, i1 true, i1 true)
; CHECK-NEXT:    call void @unknown_no_openmp()
; CHECK-NEXT:    [[C0:%.*]] = icmp eq i32 [[C]], -1
; CHECK-NEXT:    br i1 [[C0]], label [[MASTER3:%.*]], label [[EXIT:%.*]]
; CHECK:       master3:
; CHECK-NEXT:    [[Z:%.*]] = call i8* @__kmpc_alloc_shared(i64 24) #[[ATTR1]], !dbg [[DBG9:![0-9]+]]
; CHECK-NEXT:    call void @use.internalized(i8* nofree [[Z]]) #[[ATTR4]]
; CHECK-NEXT:    call void @__kmpc_free_shared(i8* [[Z]], i64 24) #[[ATTR1]]
; CHECK-NEXT:    br label [[EXIT]]
; CHECK:       exit:
