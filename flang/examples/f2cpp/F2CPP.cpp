...
public:
  UnparseVisitor(llvm::raw_ostream &out, int indentationAmount, bool capitalize,
      bool backslashEscapes, AnalyzedObjectsAsFortran *asFortran = nullptr)
      : out_{&out},
        /*langOpts_{langOpts},*/ indentationAmount_{indentationAmount},
  ...
  void Unparse(const ProgramUnit &x) { Walk(x.u); }
  ...
  void Post(const IntrinsicTypeSpec::DoublePrecision &) {
    lastType_ = "DOUBLE PRECISION";
    Word("DOUBLE PRECISION");
  }
  void Unparse(const IntrinsicTypeSpec::Character &x) {
    // Modified
    if (!x.selector) {
      lastType_ = "char";
      Put("char");
    ...
    bool IsAuto = true;
    if (const LengthSelector *y =
            std::get_if<LengthSelector>(&(*x.selector).u)) {
      common::visit(common::visitors{
                        [&](const TypeParamValue &y) {
                          if (std::get_if<ScalarIntExpr>(&y.u)) {
                            Put("CHARACTER<");
                            Walk(y);
                            Put(">");
                            IsAuto = false;
  ...
  void Unparse(const DerivedTypeStmt &x) { // R727
    ...
    assert(std::get<std::list<Name>>(x.t).empty());
    Indent();
  }
  ...
  void Unparse(const TypeParamDefStmt &x) { // R732
    // Modified
    // assert(!std::get<common::TypeParamAttr>(x.t));
    Walk(std::get<IntegerTypeSpec>(x.t));
    Walk(std::get<std::list<TypeParamDecl>>(x.t), ", ");
    Put(';');
  }
  ...
  void Unparse(const TypeBoundProcedureStmt::WithoutInterface &x) { // R749
    // Modified
    Put("operator (TODO)"), Walk(", ", x.attributes, ", ");
  ...
  void Unparse(const ImplicitStmt &x) { // R863
    common::visit(
        common::visitors{
            [&](const std::list<ImplicitSpec> &y) { assert(0); },
  ...
  void Unparse(const AssignmentStmt &x) { // R1032
    ...
    if (asFortran_ && x.typedAssignment.get()) {
      ...
      // If the left hand side of x.t is the name of the current function, then
      // return.
      auto *currentFunction =
          currentFunctions_.empty() ? nullptr : currentFunctions_.back();
      if (currentFunction) {
        auto &var = std::get<Variable>(x.t);
        if (std::holds_alternative<common::Indirection<Designator>>(var.u)) {
          auto &designator = std::get<common::Indirection<Designator>>(var.u);
          if (std::holds_alternative<DataRef>(designator.value().u)) {
  ...
  void Unparse(const CaseStmt &x) { // R1142
    const auto &selector = std::get<CaseSelector>(x.t);
    if (std::holds_alternative<Default>(selector.u)) {
      Outdent(), Word("CASE_DEFAULT");
    } else {
      Outdent(), Word("CASE "), Walk(selector);
    }
    Walk(" ", std::get<std::optional<Name>>(x.t)), Indent();
  }
  void Unparse(const EndSelectStmt &x) { // R1143 & R1151 & R1155
    Outdent(), Word("END_SELECT"), Walk(" ", x.v);
  }
  ...
  void Unparse(const PrintStmt &x) { // R1212
    ...
    auto &items = std::get<std::list<OutputItem>>(x.t);
    Walk(", ", items, ",");
    if (items.empty())
      Put(", \"\"");
    Put(");");
...
#undef FMT
  ...
  void Unparse(const ModuleStmt &x) { // R1405
    Word("MODULE (");
    Walk(x.v);
    Put(")");
    Indent();
  ...
  void Unparse(const FunctionStmt &x) { // R1530
    // Modified
    // Print the following also into a string s by temporarly using an llvm
    // string ostream as out_.
    ...
    auto nameSize = s.size() - prefixSize;
    currentFunction.prefix = std::string(s.data(), prefixSize);
  ...
  void Unparse(const SubroutineStmt &x) { // R1535
    ...
    auto nameSize = s.size() - prefixSize;
    currentFunction.prefix = std::string(s.data(), prefixSize);
    currentFunction.name = std::string(s.data() + prefixSize, nameSize);
    ...
    if (bind) {
      currentFunction.bind = &bind.value();
    }
  }
...
#include "llvm/Frontend/OpenACC/ACC.inc"
  ...
  void Unparse(const llvm::omp::Directive &x) {
    unsigned ompVersion = 50; //{langOpts_.OpenMPVersion};
...
private:
  ...
  const LengthSelector *LS{nullptr};
};
...
class F2CPP : public PluginParseTreeAction {
...
static FrontendPluginRegistry::Add<F2CPP> X("f2cpp", "print flc++");
