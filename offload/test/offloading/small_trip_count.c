...
int main() {
  // DEFAULT: Launching kernel {{.+_main_.+}} with [{{(4|2)}},1,1] blocks and [{{(32|64)}},1,1] threads in SPMD mode
...
#pragma omp target teams distribute parallel for simd
  ...
  // DEFAULT: Launching kernel {{.+_main_.+}} with [{{(5|3)}},1,1] blocks and [{{(32|64)}},1,1] threads in SPMD mode
  // EIGHT: Launching kernel {{.+_main_.+}} with [17,1,1] blocks and [8,1,1] threads in SPMD mode
#pragma omp target teams distribute parallel for simd
  for (int i = 0; i < N + 1; ++i) {
    optnone();
  }
  // DEFAULT: Launching kernel {{.+_main_.+}} with [{{(32|64)}},1,1] blocks and [4,1,1] threads in SPMD mode
  // EIGHT: Launching kernel {{.+_main_.+}} with [{{(32|64)}},1,1] blocks and [4,1,1] threads in SPMD mode
