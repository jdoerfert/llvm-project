// clang-format off
// : %libomptarget-compileoptxx-generic -O1
// : not %libomptarget-run-generic 2> %t.out
// : %fcheck-generic --check-prefixes=CHECK < %t.out
// : %libomptarget-compileoptxx-generic -O3
// : not %libomptarget-run-generic 2> %t.out
// RUN: %libomptarget-compileoptxx-generic -O3 -g -DLEVELS=11
// RUN: not %libomptarget-run-generic 2> %t.out
// %fcheck-generic --check-prefixes=DBG11 < %t.out
//  %libomptarget-compileoptxx-generic -O3 -g -DLEVELS=12
//  not %libomptarget-run-generic 2> %t.out
// RUN: %fcheck-generic --check-prefixes=DBG12 < %t.out
// clang-format on
