...
template <typename Ty, ValueKind Kind> struct Value {
  ...
  __attribute__((flatten, always_inline)) Ty &inc(IdentTy *Ident) {
    return (lookup(/* IsReadonly */ false, Ident) += 1);
  }

  __attribute__((flatten, always_inline)) Ty &dec(IdentTy *Ident) {
    return (lookup(/* IsReadonly */ false, Ident) -= 1);
  }

  __attribute__((flatten, always_inline)) Ty &set(const Ty &UpdateVal,
                                                  IdentTy *Ident) {
