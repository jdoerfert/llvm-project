...
int32_t __llvm_reduce_stage_2(void *ReduceData, void *GlobalBuffer,
                              ShuffleReductFnTy shflFct,
                              InterWarpCopyFnTy cpyFct, ListGlobalFnTy lgcpyFct,
                              ListGlobalFnTy lgredFct, ListGlobalFnTy glcpyFct,
                              ListGlobalFnTy glredFct) {
  // Copy global values into ReduceData.
  uint32_t ThreadId = mapping::getThreadIdInBlock();
  glcpyFct(GlobalBuffer, ThreadId, &ReduceData);

  // Perform a per-warp reduction.
  gpu_regular_warp_reduce(&ReduceData, shflFct);

  // Move the per-warp results to the first warp.
  uint32_t NumWarps = mapping::getNumberOfWarpsInBlock();
  cpyFct(&ReduceData, NumWarps);
