...
template <typename Ty, typename IntTy>
void reduceLeague(Ty * __restrict__ TypedDstPtr, Ty * __restrict__ TypedSrcPtr, RedConfigTy &Config,
                  int32_t) {
  ...
  if (UseProcessorIdx) {
    ...
    for (int32_t i = 0; i < NumItems; ++i)
      __builtin_nontemporal_store(TypedSrcPtr[i],
                                  &TypedDstPtr[BlockId * NumItems + i]);
...
extern "C" void __llvm_omp_default_reduction_combine_level2(
  ...
  // printf("Items %li, SrcPTr %p : DstPtr %p -> %p\n", NumItems, SrcPtr,
  // DstPtr, *((void**)DstPtr));
