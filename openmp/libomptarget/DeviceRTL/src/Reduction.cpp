...
void reduceAtomically(Ty *LHSPtr, Ty RHS, RedOpTy Op, ReducerFnTy *ReducerFn) {
  ...
  case _SUB:
    ...
    return;
  case _BIT_OR:
    atomic::bit_or((IntTy *)LHSPtr, *((IntTy *)&RHS), __ATOMIC_SEQ_CST);
    return;
  case _BIT_XOR:
    atomic::bit_xor((IntTy *)LHSPtr, *((IntTy *)&RHS), __ATOMIC_SEQ_CST);
    return;
  case _LOGIC_OR:
    break;
  case _BIT_AND:
    atomic::bit_and((IntTy *)LHSPtr, *((IntTy *)&RHS), __ATOMIC_SEQ_CST);
    return;
  case _MUL:
    atomic::mul(LHSPtr, RHS, __ATOMIC_SEQ_CST);
    return;
  case _LOGIC_AND:
    break;
  case _MAX:
    atomic::max(LHSPtr, RHS, __ATOMIC_SEQ_CST);
    return;
  case _MIN:
    atomic::min(LHSPtr, RHS, __ATOMIC_SEQ_CST);
    return;
  case _CUSTOM_OP:
...
void reduceLeagueViaTwoLevelAtomics(
  ...
  uint32_t BucketCounterValue =
      atomic::inc(BucketCounter, BucketMax, __ATOMIC_SEQ_CST);
