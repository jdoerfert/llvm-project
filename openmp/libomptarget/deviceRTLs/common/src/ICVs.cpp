...
DEVICE LazyInitArrayInSharedMem<ThreadStateTy> SHARED(omp::ThreadStates);
...
bool ICVStateTy::ensureICVStateForThread(unsigned TId) {
  if (!ThreadStates)
    ThreadStates.initialize();

  if (ThreadStates[TId])
    return false;

  ThreadStates[TId] = static_cast<ThreadStateTy*>(malloc(sizeof(ThreadStateTy)));
...
void omp_set_num_threads(int V) {
  ...
  if (!ThreadStates && TeamState.ICVState.nthreads_var == V) {
...
void ThreadStateTy::enterDataEnvironment() {
  ...
  ThreadStateTy *NewThreadState = static_cast<ThreadStateTy*>(malloc(sizeof(ThreadStateTy)));
...
unsigned omp::getThreadIdForSharedMemArrayAccess() { return GetThreadIdInBlock(); }
