...
template <typename IntTy> struct __kmpc_OmpLoop {
  static void loop(kmp_Ident *loc, IntTy global_tid, IntTy *plower,
                   IntTy *pupper, IntTy numInnerOMPLoopIterations,
                   void (*fd)(IntTy, void *), void *Payload) {
    int tid = GetLogicalThreadIdInBlock(checkSPMDMode(loc));
    omptarget_nvptx_TaskDescr *currTaskDescr = getMyTopTaskDescriptor(tid);

    // If we are explicitly bound to parallel or teams we just go ahead and use
    // those threads.
    ...
    case LoopBindKind::TEAMS:
      return omptarget_nvptx_LoopSupport<IntTy, IntTy>::dispatch_init(
          loc, global_tid, kmp_sched_distr_static_nochunk, plower, pupper, 1,
          0);
    ...
    if (!currTaskDescr->InParallelRegion()) {
      unsigned numThreads = (*pupper - *plower + numInnerOMPLoopIterations);
#pragma omp parallel num_threads(numThreads)
      impl(loc, global_tid, plower, pupper, fd, Payload,
           LoopBindKind::PARALLEL);
    } else {
...
EXTERN void
__kmpc_omp_loop_4(kmp_Ident *loc, int32_t global_tid, int32_t *plower,
                  int32_t *pupper, void (*fd)(int32_t, void *), void *Payload) {
  PRINT0(LD_IO, __FUNCTION__ "\n");
  __kmpc_OmpLoop<int32_t>::loop(loc, global_tid, plower, pupper, fd, Payload);
}
