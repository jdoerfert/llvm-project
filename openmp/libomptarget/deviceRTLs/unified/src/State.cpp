...
static constexpr uint32_t StorageTrackingBytes = 8;
...
private:
  ...
  /// Lookup the size of the storage space reserved for the warp of the
  /// encountering thread.
  /// TODO: We store this number redundantly for a lot of threads. Though,
  ///       it is packed with the storage tracker so we don't really loose
  ///       space because of it.
  uint32_t lookupWarpStorageTotal();
  ...
  /// Return the location of the total storage for this warp.
  uint32_t *getWarpStorageTotal();
...
void SharedMemorySmartStackTy::init(bool IsSPMD) {
  ...
  if (IsSPMD)
    GenericModeMainThreadStorage = 0;
  else
    GenericModeMainThreadStorage =
        config::getGenericModeMainThreadSharedMemoryStorage();

  // Initialize the tracker to the bytes used by the storage tracker and total.
  uint32_t *WarpStorageTracker = getWarpStorageTracker();
  *WarpStorageTracker = StorageTrackingBytes;

  uint32_t *WarpStorageTotal = getWarpStorageTotal();
  *WarpStorageTotal = computeWarpStorageTotal();

  static_assert(StorageTrackingBytes >=
                    (sizeof(*WarpStorageTotal) + sizeof(*WarpStorageTracker)),
                "Storage tracker bytes should cover the size of the storage "
                "tracker and total.");
}
...
void *SharedMemorySmartStackTy::push(uint64_t BytesPerLane) {
  ...
  // Determine the leader of the warp, explicit as we need the intermediate
  // values anyway
  ...
  unsigned int WarpPosition = utils::popc(Active & LaneMaskLT);
  bool IsLeader = WarpPosition == 0;
  ...
  if (IsLeader) {
    ...
    if (NumActive < mapping::getWarpSize() &&
      ...
      if (BytesInUse + BytesTotalAdjustedAligned > lookupWarpStorageTotal()) {
        Ptr = memory::allocGlobal(
            BytesTotal,
  ...
  // entire warp.
  return (char *)Ptr + (BytesPerLane * WarpPosition);
}
...
uint32_t SharedMemorySmartStackTy::computeWarpStorageTotal() {
  ...
  // uint32_t WarpStorageAligned =
  //(WarpStorageTotal / Alignment) * Alignment;
  return WarpStorageTotal;
}

uint32_t SharedMemorySmartStackTy::lookupWarpStorageTotal() {
  uint32_t *WarpStorageTotal = getWarpStorageTotal();
  return *WarpStorageTotal;
}
...
char *SharedMemorySmartStackTy::getWarpDataBottom() {
  ...
  uint32_t PriorWarpStorageTotal =
      lookupWarpStorageTotal() * mapping::getWarpId();
  if (mapping::isGenericMode())
...
uint32_t *SharedMemorySmartStackTy::getWarpStorageTracker() {
  return ((uint32_t *)getWarpDataBottom());
}

uint32_t *SharedMemorySmartStackTy::getWarpStorageTotal() {
  // The warp total is one above the tracker at the bottom of the stack.
  return getWarpStorageTracker() + 1;
}
...
void __kmpc_end_sharing_variables() { __kmpc_free_shared(GlobalArgsPtr); }
...
uint32_t &lookupForModify32Impl(uint32_t ICVStateTy::*Var) {
  ...
  if (!ThreadStates[TId]) {
    ThreadStates[TId] = static_cast<ThreadStateTy *>(memory::allocGlobal(
