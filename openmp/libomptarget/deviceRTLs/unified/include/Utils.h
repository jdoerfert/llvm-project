...
/// Return the first bit set in \p V.
inline uint32_t ffs(uint64_t V) { return __builtin_ffsl(V); }

/// Return the number of bits set in \p V.
inline uint32_t popc(uint32_t V) { return __builtin_popcount(V); }

/// Return the number of bits set in \p V.
inline uint32_t popc(uint64_t V) { return __builtin_popcountl(V); }
