...
using StreamPoolVectorTy = std::vector<std::unique_ptr<StreamPoolTy>>;
...
static CUstream getStream(const int DeviceId, __tgt_async_info *AsyncInfo,
  ...
  if (!AsyncInfo->Queue) {
    CUstream S;
    if (StreamPoolVector[DeviceId]->acquire(S) != OFFLOAD_SUCCESS)
      return nullptr;
  ...
  public:
    CUDADeviceAllocatorTy(int DeviceId, std::vector<DeviceDataTy> &DeviceData,
        ...
        : DeviceId(DeviceId), DeviceData(DeviceData),
          StreamPoolVector(StreamPoolVector) {}
    ...
    void *allocate(size_t Size, void *, TargetAllocTy Kind,
      ...
      case TARGET_ALLOC_DEVICE:
        ...
        if (AsyncInfo) {
          CUstream Stream = getStream(DeviceId, AsyncInfo, StreamPoolVector);
...
public:
  ...
  DeviceRTLTy()
    ...
    StreamPoolVector.resize(NumberOfDevices);
    ...
    for (int I = 0; I < NumberOfDevices; ++I)
      DeviceAllocators.emplace_back(I, DeviceData, StreamPoolVector);
  ...
  int initDevice(const int DeviceId) {
    ...
    if (!StreamPoolVector[DeviceId])
      StreamPoolVector[DeviceId] = std::make_unique<StreamPoolTy>(
  ...
  void *dataAlloc(const int DeviceId, const int64_t Size,
                  const TargetAllocTy Kind, __tgt_async_info *AsyncInfo) {
    switch (Kind) {
    case TARGET_ALLOC_DEFAULT:
    case TARGET_ALLOC_DEVICE:
      if (UseMemoryManager)
        return MemoryManagers[DeviceId]->allocate(Size, nullptr, AsyncInfo);
      else
  ...
  int dataExchange(int SrcDevId, const void *SrcPtr, int DstDevId, void *DstPtr,
    ...
    CUstream Stream = getStream(SrcDevId, AsyncInfo, StreamPoolVector);
  ...
  int synchronize(const int DeviceId, __tgt_async_info *AsyncInfo) const {
    ...
    // own tasks.
    StreamPoolVector[DeviceId]->release(
...
void *__tgt_rtl_data_alloc(int32_t device_id, int64_t size, void *hst_ptr,
  ...
  __tgt_async_info AsyncInfo;
  void *ptr =
      __tgt_rtl_data_alloc_async(device_id, size, hst_ptr, kind, &AsyncInfo);
  if (AsyncInfo.Queue)
...
int32_t __tgt_rtl_data_delete_async(int32_t device_id, void *tgt_ptr,
  ...
  assert(DeviceRTL.isValidDeviceId(device_id) && "device_id is invalid");
#if CUDA_VERSION < 11020
  if (async_info_ptr && async_info_ptr->Queue)
    __tgt_rtl_synchronize(device_id, async_info_ptr);
#endif
  return DeviceRTL.dataDelete(device_id, tgt_ptr, async_info_ptr);
}
