  ...
  *interop_ptr = new omp_interop_val_t(device_id, interop_type);
  ...
  if (interop_type == kmp_interop_type_tasksync) {
    ...
    __kmpc_omp_wait_deps(loc_ref, gtid, ndeps, dep_list, ndeps_noalias,
                         noalias_dep_list);
#else
    unsigned numInDeps = 0, numOutDeps = 0;
    kmp_depend_info_t InDeps[ndeps];
    kmp_depend_info_t OutDeps[ndeps];
    for (int i = 0; i < ndeps; ++i) {
      kmp_depend_info_t *dep = &dep_list[i];
      if (dep->flags.in)
        InDeps[numInDeps++] = *dep;
      if (dep->flags.out)
        InDeps[numOutDeps++] = *dep;
    }
    #pragma omp target enter data map(always,to:interop_ptr[:1])    \
                                  device((*interop_ptr)->device_id)   \
                                  depend(iterator(i=0: numInDeps), in:  InDeps[i].base_addr) \
                                  depend(iterator(i=0:numOutDeps),out: OutDeps[i].base_addr) \
                                  nowait
#endif
  }
...
EXTERN void __kmpc_interop_destroy(ident_t *loc_ref, kmp_int32 gtid,
  ...
  if (interop_val->interop_type == kmp_interop_type_tasksync) {
    ...
    #pragma omp target exit data map(from:interop_ptr[:1])    \
                                 device((*interop_ptr)->device_id)   \
                                 depend(iterator(i=0: numInDeps), in:  InDeps[i].base_addr) \
                                 depend(iterator(i=0:numOutDeps),out: OutDeps[i].base_addr) \
                                 nowait
