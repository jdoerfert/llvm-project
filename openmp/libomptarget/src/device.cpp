...
int HostDataToTargetTy::addEventIfNecessary(DeviceTy &Device,
  ...
  if (!AsyncInfo.hasPotentiallyPendingAsyncWork() || !PM->UseEventsForAtomicTransfers)
...
DeviceTy::DeviceTy(RTLInfoTy *RTL)
  ...
  SupportsAsyncFree = RTL->data_delete_async && RTL->synchronize;
}
...
TargetPointerResultTy DeviceTy::getTargetPointer(
  ...
  if (LR.Flags.IsContained ||
    ...
    // Ensure the atomicity of the allocation, if required. Allocation have to
    // look atomic even if they are async and not accompanied with a data
    // transfer.
    if (SupportsAsyncFree) {
...
void *DeviceTy::allocData(int64_t Size, void *HstPtr, int32_t Kind,
  ...
  if (!AsyncInfo || !SupportsAsyncFree)
    return RTL->data_alloc(RTLDeviceID, Size, HstPtr, Kind);
  return RTL->data_alloc_async(RTLDeviceID, Size, HstPtr, Kind, AsyncInfo);
}
