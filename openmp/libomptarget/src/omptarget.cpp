...
static int InitLibrary(DeviceTy &Device) {
  ...
  LockGuard<decltype(Device.PendingGlobalsMtx)> LG(Device.PendingGlobalsMtx);
  {
    LockGuard<decltype(PM->TrlTblMtx)> LG(PM->TrlTblMtx);
    for (auto *HostEntriesBegin : PM->HostEntriesBeginRegistrationOrder) {
      ...
      LockGuard<decltype(Device.DataMapMtx)> LG(Device.DataMapMtx);
...
static void handleDefaultTargetOffload() {
  LockGuard<decltype(PM->TargetOffloadMtx)> LG(PM->TargetOffloadMtx);
...
int targetDataBegin(ident_t *loc, DeviceTy &Device, int32_t arg_num,
  ...
  for (int32_t i = 0; i < arg_num; ++i) {
    ...
    if (arg_types[i] & OMP_TGT_MAPTYPE_PTR_AND_OBJ && !IsHostPtr) {
      ...
      if (UpdateDevPtr) {
        LockGuard<HostDataToTargetTy> LG(*Pointer_TPR.MapTableEntry);
        ...
        if (Pointer_TPR.MapTableEntry->recordEventIfNecessary(
                Device, AsyncInfo) != OFFLOAD_SUCCESS)
          return OFFLOAD_FAIL;
      } else
        Device.ShadowMtx.unlock();
    }
...
static void applyToShadowMapEntries(DeviceTy &Device, CBTy CB, void *Begin,
  ...
  LockGuard<decltype(Device.ShadowMtx)> LG(Device.ShadowMtx);
...
int targetDataEnd(ident_t *loc, DeviceTy &Device, int32_t ArgNum,
    ...
    if ((ArgTypes[I] & OMP_TGT_MAPTYPE_FROM) || DelEntry) {
      ...
      if (ArgTypes[I] & OMP_TGT_MAPTYPE_FROM) {
        ...
        if ((Always || IsLast) && !IsHostPtr) {
          ...
          LockGuard<HostDataToTargetTy> LG(*TPR.MapTableEntry);
          // Wait for any previous transfer if an event is present.
          if (void *Event = TPR.MapTableEntry->getEvent()) {
          ...
          if (IsLast) {
            if (TPR.MapTableEntry->recordEventIfNecessary(Device, AsyncInfo) !=
                OFFLOAD_SUCCESS)
              return OFFLOAD_FAIL;
          }
        }
  ...
  for (PostProcessingInfo &Info : PostProcessingPtrs) {
    ...
    // LR will be set and the DataMapMtx is being locked to avoid another thread
    // reusing the entry now.
    LookupResult LR;
    if (Info.DelEntry) {
      Device.DataMapMtx.lock();

      LR = Device.lookupMapping(Info.HstPtrBegin, Info.DataSize);
      if (LR.Entry->getTotalRefCount() != 0 ||
          LR.Entry->getDeleteThreadId() != __kmpc_global_thread_num(nullptr)) {
        Info.DelEntry = false;
    ...
    if (Info.DelEntry) {
      if (!FromMapperBase || FromMapperBase != Info.HstPtrBegin)
        Ret = Device.deallocTgtPtr(LR, Info.DataSize);

      // Done with deleting the entry, unlock the map mutex.
      Device.DataMapMtx.unlock();
