...
static void instantiateOMPDeclareVariantAttr(
  ...
  // Copy the template version of the OpenMPTraitInfo and run substitute on all
  // score and condition expressiosn.
  OpenMPTraitInfo *TI = new OpenMPTraitInfo();
  *TI = *Attr.getTraitInfos();

  for (OpenMPTraitInfo::OpenMPTraitSet &Set : TI->Sets) {
    for (OpenMPTraitInfo::OpenMPTraitSelector &Selector : Set.Selectors) {
      if (!Selector.ScoreOrCondition)
        continue;
      ExprResult ScoreOrConditionRes = Subst(Selector.ScoreOrCondition);
      if (ScoreOrConditionRes.isUsable())
        Selector.ScoreOrCondition = ScoreOrConditionRes.get();
      else
